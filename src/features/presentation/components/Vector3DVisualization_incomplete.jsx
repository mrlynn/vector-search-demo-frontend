...
const Vector3DVisualization = () => {
  ...
  const initializeForceLayout = () => {
    const nodes = [];
    const links = [];
    const nodeMap = new Map(); // Keep track of created nodes
    ...
    // Then create links only between existing nodes
    ...
    console.log('Nodes:', nodes.length, 'Links:', links.length); // Debug info
    ...
    simulation.on('tick', () => {
      nodes.forEach(node => {
        ...
        if (point?.mesh) {
          // Scale positions to match our scene
      ...
      onClick={() => {
        ...
        if (sim) {
          console.log('Simulation:', {
            nodes: sim.nodes(),
            links: sim.force('link').links(),
            alpha: sim.alpha(),
            running: sim.alpha() > sim.alphaMin()
          });
        }
      }}
  ...
  const createSemanticClouds = () => {
    ...
    Object.entries(VECTOR_CLUSTERS).forEach(([clusterKey, cluster]) => {
      ...
      if (positions.length > 0) {
        ...
        const radius = Math.max(...positions.map(pos => pos.distanceTo(center))) + 0.5;
  ...
  useEffect(() => {
    ...
    const clouds = createSemanticClouds();
    ...
    function animate() {
      ...
      // Update line animations if any exist
      if (activeLines.current.length > 0) {
        animateLines();
      }
    ...
    return (
      <div className="flex flex-col gap-2 mt-4">
        <button
          ...
          className="px-4 py-2 bg-white rounded-lg shadow-sm hover:bg-gray-50 text-sm"
        >
          {isLayoutActive ? 'Stop' : 'Start'} Force Layout
        </button>
        <DebugForceLayout />
      </div>
    );
  ...
  // Only update positions if simulation is active
  if (simulationRef.current && simulationRef.current.alpha() > simulationRef.current.alphaMin()) {
    const nodes = simulationRef.current.nodes();
    nodes.forEach(node => {
      const point = pointsRef.current[node.word];
